...
function getPizza() {
  ...
  fetch(`/api/pizzas/${pizzaId}`)
    ...
    .then(printPizza)
    // Now add a .catch() so that any error takes the user back to the home page, 
    // using the window.history.back() method. 
    .catch(err => {
      console.log(err);
      ...
      // As long as this particular browser session has a previous page, it will behave as if the user 
      // had clicked on the "Back" button.
      window.history.back();
    });
}
